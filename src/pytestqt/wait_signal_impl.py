...
from pytestqt.exceptions import TimeoutError, CallbackCalledTwiceError
from pytestqt.utils import SignalAndArgs
from pytestqt.qt_compat import qt_api


if not hasattr(qt_api, "QtCore"):
    raise ImportError(
        "wait_signal_impl.py got imported too early (before qt_api is initialized)! "
        "To access the [...]Blocker classes for type annotations, "
        "guard the imports with `if TYPE_CHECKING:`. For any other usage, "
        "use `from pytestqt import wait_signal` and access `wait_signal.[...]Blocker`."
    )
